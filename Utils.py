...
def get_splits(sampling: str = 'imbalanced'):
    ...
    X_temp, X_test, y_temp, y_test = train_test_split(X, y, test_size=0.10, random_state=1, stratify=y)
...
def train_and_evaluate_all(sampling: str = 'imbalanced'):
    ...
    # Gradient Boosting
    gbc = GradientBoostingClassifier(n_estimators=50, learning_rate=1.0, max_depth=1, random_state=0).fit(X_train, y_train)
    results['GBC'] = _eval_model(gbc, X_test, y_test)

    # NN
    nn = _build_nn(X_train.shape[1])
...
def predict_single(sampling: str = 'imbalanced', input_array: np.ndarray = None, **input_features):
    ...
    X_train, X_val, _, y_train, y_val, _ = get_splits(sampling)
    ...
    if best_name == 'LR':
        ...
        model = _build_nn(X_full.shape[1])
