...
if input_method == "Paste Array":
    ...
    if len(parts) == len(features):
        try:
            input_array = [float(x) for x in parts]
            input_features = dict(zip(features, input_array))
        except:
            st.sidebar.error("Invalid number format in array.")
    else:
    ...
    for feat in features:
        ...
        input_features[feat] = st.sidebar.number_input(feat, value=default)

# Predict & evaluate
go = st.button("🚀 Predict & Evaluate")
if go:
    if not input_features:
        st.error("Please provide valid input.")
    else:

        best_model, pred = predict_single(sampling='imbalanced', input_array=input_array, **input_features)
        ...
        for i, strat in enumerate(strategies):
            with cols[i]:
                ...
                table = {name: {k: f"{v:.3f}" for k,v in res.items() if k!='cm'}
        ...
        for strat, results in all_results.items():
            for name, r in results.items():
                if r['f1'] > best[1]: best = (f"{name} on {strat}", r['f1'])
